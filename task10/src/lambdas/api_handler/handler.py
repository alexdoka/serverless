...
class ApiHandler():
    ...
    def handle_request(self, event, context):
        ...
        if http_method == 'POST' and path == '/reservations':
            ...
            #     }
            # print(response)
            # Filter reservations for the same date and check for time overlap
            # for item in response['Items']:
            #     if item['date']['S'] == date:
            #         existing_start = datetime.strptime(item['slotTimeStart']['S'], '%H:%M')
            #         existing_end = datetime.strptime(item['slotTimeEnd']['S'], '%H:%M')
            #         table_id = item['tableNumber']['N']
            #         if table_id == table_number:
            #             if (max(existing_start, slot_time_start) < min(existing_end, slot_time_end)):
            #                 return {
            #                     'statusCode': 400,
            #                     'body': 'Time slot is already reserved'
            #                 }
